...
**1. Firm/regulatory entity disclosure facts.** The footer disclosure block needs the approved firm name (already resolved -- "Reserve Investment Group, Inc," per `MISSING-INFORMATION-REGISTER.md` item 1, though `DISCLOSURE-INVENTORY.md` still shows this as open and should be updated), plus the broker-dealer disclosure and investment-adviser disclosure language. Per `MISSING-INFORMATION-REGISTER.md` items 2 through 4, the practice owner's Edward Jones affiliation is confirmed but Reserve Investment Group must stay fully separate from Edward Jones in any on-site branding, and it's still unresolved whether RIG's own planning/insurance activity is conducted through an RIA, and if so which one, with CRD/IARD numbers still unsupplied. Because this disclosure lives in the sitewide footer, it is very likely the single largest blocker on the list -- it touches essentially every page.

**2. Named broker-dealer / RIA compliance reviewer.** `HUMAN-APPROVAL-MATRIX.md` requires a "designated broker-dealer principal or compliance reviewer" and an "authorized RIA compliance reviewer" to sign off before any page referencing brokerage or advisory services can be considered approved. Neither role has been named yet. The existing "standing draft-copy approval" from the practice owner covers tone and copy-as-drafted, but the matrix is explicit that it does not substitute for this sign-off. Even a page with zero remaining bracket placeholders cannot reach "Production approved" without this.

**3. Form CRS link.** `MISSING-INFORMATION-REGISTER.md` item 9, still open. Needed in the footer, About page, and Disclosures page.
